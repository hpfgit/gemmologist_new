  ...
  <view class="container">
    <view class="navs-box">
      <view class="navs">
        <view
          ...
          @tap="tabNav(0)"
        >
          <image src="../../static/images/equipmentappraisal/鞋@2x.png"></image>
          <text>球鞋</text>
        </view>
        <view
          class="nav"
          ...
          @tap="tabNav(1)"
        >
          <image src="../../static/images/equipmentappraisal/服装@2x.png"></image>
          <text>服装</text>
        </view>
      </view>
    </view>
    <view class="brands" v-show="type === 'qx'">
      ...
      <view class="brand-box">
        <view class="box" v-for="(item, index) in xie" :key="index" @tap="goTo(item.id)">
          <view class="left">
            <image class="cover_image" :src="getPath(item.cover_image)"></image>
    ...
    <view class="brands clothing" v-show="type === 'fz'">
      ...
      <view class="brand-box">
        <view class="box" v-show="tabIndex === 0" v-for="(item, index) in clothing" :key="index" @tap="goTo(item.id)">
          <view class="left">
            <image class="cover_image" :src="getPath(item.cover_image)"></image>
            <view class="name">{{item.name}}</view>
          </view>
...
export default {
  data() {
    return {
      ...
      is_specialty: uni.getStorageSync("is_specialty") || "",
      tabIndex: 0,
      type: ''
    };
  },
  onLoad(options) {
    ...
    uni.setStorageSync("is_specialty", is_specialty);
  ...
  methods: {
    tabs(index) {
      this.tabIndex = index;
      if (index) {
        clothing({ page: 1, type: 1, limit: 1000, is_sport: 0 }).then(result => {
          const { data } = result.data;
          this.clothing = data;
          uni.hideLoading();
          console.log(result);
    ...
    goTo(id) {
      uni.navigateTo({
        url:
          "/pages/selectappraiser/selectappraiser?brand_id=" +
          id
      });
    }
  }
