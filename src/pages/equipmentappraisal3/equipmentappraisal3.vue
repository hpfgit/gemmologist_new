<template>
  <view class="container">
    ...
    <view class="brands" v-show="currentIndex === 0">
      <view class="box" v-for="(item, index) in xie" :key="index">
        <view @tap="goTo(item.id)">
          <image :src="getPath(item.cover_image)"></image>
        </view>
      </view>
    </view>
    <view class="brands clothing" v-show="currentIndex === 1">
      <view class="box" v-for="(item, index) in clothing" :key="index">
        <view @tap="goTo(item.id)">
          <image :src="getPath(item.cover_image)"></image>
        </view>
      </view>
...
export default {
  ...
  methods: {
    ...
    goTo(id) {
      if (parseInt(this.is_specialty)) {
        uni.navigateTo({
          url:
            "/pages/examination/examination?brand_id=" +
            id +
            "&is_specialty=" +
            this.is_specialty
        });
